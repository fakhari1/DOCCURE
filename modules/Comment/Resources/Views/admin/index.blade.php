...
@section('breadcrumb')
    @include('Dashboard::admin.sections.breadcrumb', ['data' => ['نظرات سایت']])
@endsection

@section('content')

    <h3 class="mb-4">نظرات سایت</h3>
        ...
        @else
            @foreach($comments as $key => $comment)
                <tr>
                    ...
                    <td class="text-center">
                        <a href="{{ route('admin.comments.update-status', $comment) }}" onclick="event.preventDefault(); document.getElementById('form_status').submit();">
                            {!! $comment->status_badge !!}
                        </a>
                        <form action="{{ route('admin.comments.update-status', $comment)  }}" method="post" id="form_status">
                            @csrf
                        </form>
                    </td>
                    <td class="text-center">
...
{{--                        <a href="{{ route('admin.comments.delete', ['comment' => $comment]) }}"--}}
